...
The `public contributions` by users in Australia on `2024/1/10 2:01 AM UTC`. This list contains users from `Australia` and cities `Sydney` `Melbourne` `Perth` `Adelaide` `Brisbane` `Canberra` `Hobart` `Gold-coast` `Darwin`.

There are `9 countries` and `68 cities` can be found [here](https://github.com/kyalo3/top-github-users).

There are `969 users`  in Australia. You need at least `115 followers` to be on this list.
